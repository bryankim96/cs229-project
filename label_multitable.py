...
def label_data(read_path_1, read_path_2, read_path_3, write_path):
    ...
    response_df1['RESPONSE1'] = df2.apply(lambda row: good_results_tumor(row['CS_SITE_SPEC_F16'], row['CS_SITE_SPEC_F21'], row['TCODE_P'], row['NCODE_P'], row['DERIVEDAJCC7T'], row['DERIVEDAJCC7N']), axis = 1)
...
if __name__ == "__main__":
    label_data('../haruka_radiology_reports_111618.csv', '../V4_S_CCR_TUMOR.csv', '../surv_time.csv', '../time_labeled_rad_reports.csv')