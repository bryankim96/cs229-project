...
def good_results_exist(target_id, target_time, row_id, row_time, response):
    ...
    later_date = int(row_year) > int(target_year) or (int(row_year) == int(target_year) and (
            int(row_month) > int(target_month) or (
            int(row_month) == int(target_month) and int(row_day) > int(target_day))))
...
def label(row_id, row_time, df):
    print(row_id)
    if True in df.apply(
            lambda row: good_results_exist(row_id, row_time, row['ANON_ID'], row['RESULT_TIME'], row['GOOD_RESPONSE']),
            axis=1).values:
        return 1
    elif True in df.apply(
            lambda row: bad_results_exist(row_id, row_time, row['ANON_ID'], row['RESULT_TIME'], row['GOOD_RESPONSE']),
            axis=1).values:
        return -1
    else:
        return 0
...
if __name__ == "__main__":
    label_data('../haruka_pathology_reports_111618.csv', '../haruka_radiology_reports_111618.csv',
               '../labeled_radiology_reports.csv')
